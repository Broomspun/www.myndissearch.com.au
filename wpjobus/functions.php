...
function searchformcarer(){
	?>
	<form action="<?php echo home_url('/'); ?>search-ndis-provider" method="get" id="search-resumes-form" accept-charset="UTF-8">

							            		<div class="column_box first">
							            		...
							            		<div class="column_box">
							            			...
							            			<select name="resume_type" id="job_type" style="width: 100%; margin-bottom: 0;">
							            				<option value='' ><?php _e( 'Any type', 'themesdojo' ); ?></option>
														<?php 
															global $redux_demo, $td_job_location; 
							            		...
							            		<div class="column_box" style="margin-bottom: 0">

							            			<span id="comp-reset" class="button-ag-full homepage-search-button sumiform" style="width: 100%; text-align: center;" ><i class="fa fa-search"></i><?php _e( 'Search', 'themesdojo' ); ?></span>
							            	...
							            	<script type="text/javascript">
							            		jQuery(document).ready(function(){
												    jQuery(".sumiform").click(function(){
												        jQuery(this).closest("form").submit();

												    });
												    setTimeout(function(){ jQuery(".availabilty_cal_provider").val(""); }, 1000);
												});
							            	</script>
	<?php
}
function searchformjob(){
	?>
	<form action="<?php echo home_url('/'); ?>search-ndis-jobs" method="get" id="search-jobs-form" accept-charset="UTF-8">
												...
												<div class="column_box">
							            			...
							            			<select name="job_presence" id="job_presence" style="width: 100%; margin-bottom: 0;">
							            				<option value='' ><?php _e( 'All work type', 'themesdojo' ); ?></option>
														<?php 
															global $redux_demo, $td_job_location; 
															for ($i = 0; $i < count($redux_demo['job_presence_type']); $i++) {
														?>
														<option value='<?php echo $redux_demo['job_presence_type'][$i]; ?>' ><?php echo $redux_demo['job_presence_type'][$i]; ?></option>
														<?php 
															}
														?>
													</select>
							...
							<?php 
									...
									<?php
							            		...
							            		<div class="column_box" style="margin-bottom: 0">
							            				...
							            				<option value='' ><?php _e( 'Any Service', 'themesdojo' ); ?></option>
															...
															<option value='High Intensity Daily Personal Activities'>High Intensity Daily Personal Activities</option>
															...
															<option value='Personal Activities High' >Personal Activities High</option>
															...
															<option value='Specialised Disability Accommodation' >Specialised Disability Accommodation</option>
															...
															<option value='Specialised Hearing Services'  >Specialised Hearing Services</option>
							            		...
							            		<input type="text" class="form-control availabilty_cal_provider" name="availability" id="availabilty_cal_provider" value="" placeholder="Availability">
												...
												<div class="column_box" style="margin-bottom: 0">
							            					...
							            					<select  name="company_rating" id="company_rating" style="width: 100%; margin-bottom: 0;">
							            				<option value='' ><?php _e( 'Any Rating', 'themesdojo' ); ?></option>
							            		...
							            		<div class="" style="margin-bottom: 0">
							            			...
							            			<span  class="sumiform" id="comp-reset" class="button-ag-full homepage-search-button" style="width: 100%; text-align: center;" ><i class="fa fa-search"></i><?php _e( 'Search', 'themesdojo' ); ?></span>
...
add_action( 'my_action_for_run_product', 'process_posta' );
...
function process_posta() {

     	

     	$file = fopen("https://www.myndissearch.com.au/wp-content/uploads/2017/06/aaa.csv","r");
		...
		foreach ($csv_arr as $key => $value) {
			...
			$wpjobus_company_services=array();
			...
			foreach ($value as $keys => $values) {
				...
				if($values[1]!=''){
					...
					$headoffname=$values[1];

				}

				if($values[2]!=''){

					$wpjobus_company_services[] = array($values[2],0,0);
			...
			$post_information = array(
		  	...
		  	echo $td_post_id = wp_insert_post($post_information);

		  	update_post_meta($td_post_id, 'wpjobus_company_fullname', $compname);

			update_post_meta($td_post_id, 'wpjobus_post_title', $compname);

			update_post_meta($td_post_id, 'wpjobus_company_services', $wpjobus_company_services);

			update_post_meta($td_post_id, 'wpjobus_company_phone', $phonenum);

			update_post_meta($td_post_id, 'wpjobus_company_website', $website);

			update_post_meta($td_post_id, 'wpjobus_company_email', $email);

			update_post_meta($td_post_id, 'wpjobus_company_address', $address);
...
if( !class_exists('WPJobus_custom_twitter') ){
...
function WPJobus_scripts_styles() {
	...
	//Adding custom master css

	wp_register_style( 'custom-master-css', get_template_directory_uri() . '/css/master.css' );

	wp_enqueue_style( 'custom-master-css' );

	// Loads JavaScript file with functionality specific to WPJobus.

	wp_enqueue_script( 'WPJobus-script', get_template_directory_uri() . '/js/functions.js', array( 'jquery' ), true );
	...
	wp_enqueue_style( 'WPJobus-style', get_stylesheet_uri(), array() );
	...
	wp_enqueue_script( 'placesdojo-job-manager-map', get_template_directory_uri() . '/js/populate-map.js' );



	$settings = array(

		'url_icon' 	=> get_template_directory_uri() .'/images/icon-map.png',

		'url_theme' => get_template_directory_uri(),

		'results_no' => esc_html__( 'Results', 'placesdojo'),

	);



	wp_localize_script( 'placesdojo-job-manager-map', 'placesdojoMapSettings', apply_filters( 'placesdojo_map_settings', $settings ) );

	

	//wp_enqueue_script( 'google-maps', 'https://maps.googleapis.com/maps/api/js', array( 'jquery' ), true );

	wp_enqueue_script( 'google-maps', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDSkRsX1FYp6aYqZyCT8N-sVKvBs2ptZMg', array( 'jquery' ), true );
	...
	wp_enqueue_script( 'gmap3',       get_template_directory_uri() . '/js/gmap3.js', array('jquery'), '5.1.1' , true );

	wp_enqueue_script( 'infobox',  get_template_directory_uri() . '/js/infobox.js', array('jquery'), '1.0', true );

	wp_enqueue_script( 'markerwithlabel',  get_template_directory_uri() . '/js/markerwithlabel.js', array('jquery'), '1.0', true );

	wp_enqueue_script( 'markerclusterer',  get_template_directory_uri() . '/js/markerclusterer.js', array('jquery'), '1.0', true );

	wp_enqueue_script( 'oms', get_template_directory_uri() . '/js/oms.min.js', array( 'jquery' ), true );
	...
	wp_enqueue_script( 'WPJobus-easing-script', get_template_directory_uri() . '/js/jquery.easing.1.3.js', array( 'jquery' ), true );



	// Load form script

	wp_enqueue_script( 'WPJobus-form-script', get_template_directory_uri() . '/js/jquery.form.js', array( 'jquery' ), true );



	// Load validate script

	wp_enqueue_script( 'WPJobus-validate-script', get_template_directory_uri() . '/js/jquery.validate.min.js', array( 'jquery' ), true );



	// Load isotope script

	wp_enqueue_script( 'WPJobus-isotope-script', get_template_directory_uri() . '/js/jquery.isotope.min.js', array( 'jquery' ), true );



	// Load jQuery UI script

	wp_enqueue_script( 'WPJobus-jquery-ui-script', '//code.jquery.com/ui/1.11.4/jquery-ui.js', array( 'jquery' ), true );



	// Load jQuery tools forms script

	wp_enqueue_script( 'WPJobus-jquery-tools-forms-script', get_template_directory_uri() . '/js/jquery.tools.min.js', array( 'jquery' ), true );



	// Load jQuery ui script

	wp_enqueue_script( 'WPJobus-jquery-tools-forms-script', get_template_directory_uri() . '/js/jquery-ui.min.js', array( 'jquery' ), true );
	...
	wp_enqueue_script( 'admin-gmap-script', get_template_directory_uri() . '/js/gmap3.min.js', array( 'jquery' ), true );
	...
	if ( is_page_template('template-add-company.php') or is_page_template('template-add-resume.php') or is_page_template('template-add-job.php') ) {
	  	...
	  	wp_enqueue_script( 'admin-geo-google-map', get_template_directory_uri() . '/js/getlocation-map-script.js', array( 'jquery' ), true );



	  	wp_enqueue_script( 'admin-jquery-ui-script', '//code.jquery.com/ui/1.11.4/jquery-ui.js', array( 'jquery' ), true );



	}
	if(is_page_template('companysinglefile.php') || is_singular('company'))
	{
		wp_enqueue_script( 'photogallery-js', get_template_directory_uri() . '/js/photo-gallery.js', '', '',true );
	}  
	...
	if( is_single() ) {
		...
		wp_enqueue_script( 'WPJobus-flexslider-script', get_template_directory_uri() . '/js/jquery.flexslider.js', array( 'jquery' ), true );
	...
	wp_enqueue_script( 'WPJobus-sticktop-script', get_template_directory_uri() . '/js/stickTop.js', array( 'jquery' ), true );



	// Load menu script

	wp_enqueue_script( 'WPJobus-custom-menu', get_template_directory_uri() . '/js/menu.js', array( 'jquery' ), true );



	 // Load like script

	wp_enqueue_script( 'WPJobus-like-script', get_template_directory_uri() . '/inc/like/i-like-this.js', array( 'jquery' ), true );
	...
	wp_enqueue_script( 'WPJobus-inview-script', get_template_directory_uri() . '/js/jquery.inview.js', array( 'jquery' ), true );



	// Load lightbox script

	wp_enqueue_script( 'WPJobus-lightbox-script', get_template_directory_uri() . '/js/lightbox.min.js', array( 'jquery' ), true );



	// Load custom script

	wp_enqueue_script( 'WPJobus-custom-script', get_template_directory_uri() . '/js/custom.js', array( 'jquery' ), true );



	// Load Stripe script

	wp_enqueue_script( 'WPJobus-stripe-script', 'https://checkout.stripe.com/checkout.js', array( 'jquery' ), true );





	wp_enqueue_script( 'WPJobus-imag-gallery-carousel', get_template_directory_uri() . '/js/jquery.carouFredSel-6.2.1-packed.js', array( 'jquery' ), true );



	

	wp_enqueue_script( 'WPJobus-classie', get_template_directory_uri() . '/js/classie.js', array( 'jquery' ), true );

	wp_enqueue_script( 'WPJobus-anime-by-scroll', get_template_directory_uri() . '/js/AnimOnScroll.js', array( 'jquery' ), true );



	// Load owl script

	wp_enqueue_script( 'WPJobus-owl-carousel', get_template_directory_uri() . '/js/owl.carousel.js', array( 'jquery' ), true );
...
function WPJobus_admin_enqueue() {
	...
	wp_enqueue_script( 'admin-script', get_template_directory_uri() . '/inc/admin/admin_script.js', array( 'jquery' ), true );



	wp_enqueue_script( 'admin-google-maps-script', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyDSkRsX1FYp6aYqZyCT8N-sVKvBs2ptZMg', array( 'jquery' ), true );
...
/*	global $post_type;
	...
	wp_enqueue_script( 'admin-jquery-ui-script', '//code.jquery.com/ui/1.10.4/jquery-ui.js', array( 'jquery' ), true );*/
...
function wpcook_load_custom_wp_admin_style() {
	...
	wp_enqueue_script( 'WPJobus-form-admin-script', get_template_directory_uri() . '/js/jquery.form.js', array( 'jquery' ), true );
...
add_action( 'woocommerce_add_to_cart',  'themeprefix_add_to_cart_redirect', 11 );
...
function provider_purchase_code_func( $atts ) {

	$value = '<link type="text/css" href="'.esc_url( get_template_directory_uri() ).'/style-table.css" rel="stylesheet" />';

	$params = array('posts_per_page' => 2, 'post_type' => 'product','post__in' => array('3421','3420') );

	$wc_query = new WP_Query($params);

	if ($wc_query->have_posts()) : 

		$value .= '<section class="layer plans"><section>';

		while ($wc_query->have_posts()) :  $wc_query->the_post(); 

			$product = get_product(get_the_ID());

				$value .= '<section class="third lift plan-tier" >';

				if($product->featured == 'yes'){ 

					$value .= '<h6>Most popular</h6>';

				}  

				if($product->featured == 'yes'){ 

					$value .= '<h4 class="featured-box">'.get_the_title().'</h4>';

				}else{ 

					$value .= '<h4>'.get_the_title().'</h4>';

				}

				$value .= '<h5><sup class="superscript">$</sup><span class="plan-price">';
				...
				$value .= get_the_content();
					...
					$value .= '<form class="cart" method="post" action="'.home_url('/register').'">								
				...
				$value .= '<div style="clear: both"></div></section></section>';

				wp_reset_postdata();

				else:

				$value .= '<p>'. _e( 'No Products').'</p>';
...
add_shortcode( 'provider_purchase_code', 'provider_purchase_code_func' );





function carer_purchase_code_func( $atts ) {

	$value = '<link type="text/css" href="'.esc_url( get_template_directory_uri() ).'/style-table.css" rel="stylesheet" />';

	$params = array('posts_per_page' => 1, 'post_type' => 'product','post__in' => array('3421'), 'order' => 'ASC', 'orderby' => 'ID' );
...
add_shortcode( 'carer_purchase_code', 'carer_purchase_code_func' );
...
function provider_package_func( $atts ) {

	$value = '<link type="text/css" href="'.esc_url( get_template_directory_uri() ).'/style-table.css" rel="stylesheet" />';

	$params = array('posts_per_page' => 3, 'post_type' => 'product','post__in' => array('3875','3877','3878'),'order' => 'ASC', 'orderby' => 'ID' );
	...
	if ($wc_query->have_posts()) : 
		...
		while ($wc_query->have_posts()) :  $wc_query->the_post(); 
			...
			$product = get_product(get_the_ID());
				...
				$value .= '<section class="four lift plan-tier" >';
				...
				$value .= ' <sub> /year</sub></span><sub></sub></h5><del>';



				if($product->regular_price != ""){

					$value .= '$'.$product->regular_price.'/year';
				...
				$value .= get_the_content();
					...
					$value .= '	   <form class="cart" method="post" enctype="multipart/form-data">

							 <input type="hidden" name="add-to-cart" value="'.esc_attr($product->id).'">

							 <button type="submit">'. $product->single_add_to_cart_text().' </button>

						</form>';*/

				$value .= '<form class="cart" method="post" action="'.home_url('/basket?').'add-to-cart='.esc_attr($product->id).'">								
...
add_shortcode( 'provider_package', 'provider_package_func' );



function provider_package_all_func( $atts ) {

	$value = '<link type="text/css" href="'.esc_url( get_template_directory_uri() ).'/style-table.css" rel="stylesheet" />';

	$params = array('posts_per_page' => 4, 'post_type' => 'product','post__in' => array('3548','3875','3877','3878'),'order' => 'ASC', 'orderby' => 'ID' );

	$wc_query = new WP_Query($params);

	if ($wc_query->have_posts()) : 

		$value .= '<section class="layer-new plans"><section>';

		while ($wc_query->have_posts()) :  $wc_query->the_post(); 

			$product = get_product(get_the_ID());

				$value .= '<section class="five lift plan-tier" >';
...
add_shortcode( 'provider_package_all', 'provider_package_all_func' );


function get_provider_package_plan(){	
	if ( is_user_logged_in() ){
		$current_user = wp_get_current_user();
		$user_login_id = $current_user->ID;
		$key = 'account_type';
		$single = true;
		$user_account_type = get_user_meta( $user_login_id, $key, $single );
		if($user_account_type == 'job-offer'){			
			$user_account_plan = get_user_meta( $user_login_id, 'wsspg_test_subscriptions', $single );
			if(empty($user_account_plan)){
				 return 'Free Listing';			
			}else{
				$memeberid = end($user_account_plan);
				if($memeberid == 'memberplan10005'){
					return 'Gold Membership';
				}else if($memeberid == 'memberplan10001'){
					return 'Silver Membership';
				}else if($memeberid == 'memberplan10003'){
...
function your_hidden_data( $order_id ) {
    ...
    */
	...
	update_user_meta($user_login_id, 'account_type', 'job-offer');


}

function get_post_user_plan($user_id){
			$user_account_plan = get_user_meta( $user_id, 'wsspg_test_subscriptions', $single );
			if(empty($user_account_plan)){
				 return 'Free Listing';			
			}else{
				$memeberid = end($user_account_plan);		
				if($memeberid[0] == 'memberplan10005'){
					return 'Gold Membership';
				}else if($memeberid[0] == 'memberplan10001'){
					return 'Silver Membership';
				}else if($memeberid[0] == 'memberplan10003'){
					return 'Platinum Membership';
				}				
			}
}
 ...
 add_action( 'wp_ajax_add_providers_ratings', 'add_providers_ratings_callback' );
 add_action( 'wp_ajax_nopriv_add_providers_ratings', 'add_providers_ratings_callback' );



 function add_providers_ratings_callback(){
	 ...
	 if(!empty($_POST['post_id'])){
		$current_post_id = trim($_POST['post_id']);
	 }
	 $current_time = date('Y-m-d');
	  ...
	  $meta_key_total = 'provider_'.$key.'_rating_total';
	  $meta_key_average = 'provider_'.$key.'_rating_average';
	 ...
	 if ( is_user_logged_in() ){
		...
		global $wpdb;						
		$current_user_posts = $wpdb->get_results( "select id from wp_provider_rating where user_id = '".$login_user_id."' and post_id = '".$current_post_id."' and rating = '".$rating."' limit 0,2");
		...
		if(empty($current_user_posts)){
			 ...
			 update_post_meta( $current_post_id, $meta_key_average, $average );
			 update_post_meta( $current_post_id, $meta_key_total, $count );

			if($average != ''){
				$get_value_int_average = floor($average);
			}else{
				$get_value_int_average = 0;
			}
			$get_value_total = $count;
			$value_response = array();
			$value_response['type'] =3;
			$value_response['type_id'] =$id;
			$value_response['average'] =$average;
			$value_response['rating'] =$get_value_int_average;
			$value_response['total'] =$get_value_total;
			$value_response['message'] ='Thank you.';

			echo json_encode($value_response);

		}else{
			$get_value_average = get_post_meta( $current_post_id, $meta_key_average, true);
			...
			$value_response = array();
			$value_response['type'] =2;
			$value_response['type_id'] =$id;
			$value_response['average'] =$get_value_average;
			$value_response['rating'] =$get_value_int_average;
			$value_response['total'] =$get_value_total;
			$value_response['message'] ='You have already add rating for this provider.';
			echo json_encode($value_response);
		}
			...
			$get_value_total = get_post_meta( $current_post_id, $meta_key_total, true);
			$value_response = array();
			$value_response['type'] =1;
			$value_response['type_id'] =$id;
			$value_response['average'] =$get_value_average;
			$value_response['rating'] =$get_value_int_average;
			$value_response['total'] =$get_value_total;
			$value_response['message'] ='Please login to add ratings.';
			echo json_encode($value_response);
	}
...
//Add custom column to company post type

add_filter('manage_company_posts_columns', 'ST4_columns_company_head');
add_action('manage_company_posts_custom_column', 'ST4_columns_company_content', 10, 2);
// ADD NEW COLUMN
function ST4_columns_company_head($defaults) {
    $defaults['featured_profile'] = 'Feature the Profile';
    return $defaults;
}
 
// SHOW THE FEATURED IMAGE
function ST4_columns_company_content($column_name, $post_ID) {
	
	$companystatus=get_post_status($post_ID);
    
	if ($column_name == 'featured_profile') {
		if($companystatus=='publish')
		{
		$job_star_status = get_post_meta( $post_ID, '_featured', true );
		$onclass="";
		
        $job_star_image = get_template_directory_uri().'/img/star_off.png';
		if($job_star_status==1)
		{
		$job_star_image = get_template_directory_uri().'/img/star_on.png';
		$onclass=" on";
		}
		
       
            echo '<img id="'.$post_ID.'" class="img-swap'.$onclass.'" src="' . $job_star_image . '" />';
...
add_action( 'admin_head', 'js_featured_jobs' );
function js_featured_jobs()
{
	?> 
	<script>
			jQuery(document).ready(function(){
				
				  jQuery(".img-swap").live('click', function() {
					  var myjobid=jQuery(this).attr("id");
					  var selectedjob='no';
					 //alert(myjobid);
...
add_action( 'wp_ajax_job_featured_ajax', 'user_featured_ajax' );
add_action( 'wp_ajax_nopriv_job_featured_ajax', 'user_featured_ajax' );

function user_featured_ajax()
{
$selectedjob=$_POST['selectedjob'];
$myjobid=$_POST['myjobid'];
if(isset($selectedjob) && isset($myjobid))
{	
if($selectedjob=='yes')
{   
   update_post_meta($myjobid, '_featured', '1');
}
elseif($selectedjob=='no')
{
   update_post_meta($myjobid, '_featured', '0');
}
...
add_shortcode('featured_company_profiles','featured_company_profiles_cb');
function featured_company_profiles_cb()
{
	$company_featured='';
			$t=0;
			$args=array('post_type'=>'company','posts_per_page'=>-1,'meta_query' => array(
			array(
			'key'     => '_featured',
			'value'   => 1,
			'compare' => '=',
			...
			$featured_profile_status=get_option('featured_profile_status');
			$get_post_user_plan = get_post_user_plan(get_current_user_id());
			if($featured_profile_status=='yes' && $get_post_user_plan=='Silver Membership')
			{
			$company_featured .='<section class="">
		<div class="container"><div class="featured_company_profile_out">
			<h2>Featured Providers</h2>';
			
			if ( $the_query->have_posts() ) {
			$company_featured .='<div id="owl-car-1" class="owl-carousel owl-theme">';
			while ( $the_query->have_posts() ) {
				$the_query->the_post();
				$companyid=$the_query->post->ID;
				$wpjobus_company_fullname = esc_attr(get_post_meta($companyid, 'wpjobus_company_fullname',true));
				$wpjobus_company_address = esc_attr(get_post_meta($companyid, 'wpjobus_company_address',true));
				$wpjobus_company_phone = esc_attr(get_post_meta($companyid, 'wpjobus_company_phone',true));
				$wpjobus_company_services = get_post_meta($companyid, 'wpjobus_company_services',true);
				$wpjobus_company_profile_picture = esc_attr(get_post_meta($companyid, 'wpjobus_company_profile_picture',true));
            $company_featured .='<div class="item"><div class="ftured-sec"><div class="logo-head">';
            if($wpjobus_company_profile_picture)
			  {
            $company_featured .='<div class="fts-logo">
            		
			  <img src="'.$wpjobus_company_profile_picture.'">
			 
            </div>';
             }
             else
             {
             	 $company_featured .='<div class="fts-logo-nologo">
             	 </div>';

             }
              $company_featured .='<h4><a href="'.get_permalink($companyid).'">'.$wpjobus_company_fullname.'</a></h4></div>
			  <div class="ovrflw-scrll"><div class="contactout">
			  <span class="fs-add">'.$wpjobus_company_address.'</span>
			  <span class="fs-phone"><a href="'.$wpjobus_company_phone.'">'.$wpjobus_company_phone.'</a></span>
			  </div>';
			  if(!empty($wpjobus_company_services)) {
			  $company_featured .='<div class="featured_services_out">';
			  
			  for ($i = 0; $i < (count($wpjobus_company_services)); $i++) {
				  
				  $company_featured .='<span class="srvce-spn">';
				  if($i>0)
				  {
					  $company_featured .='';
				  }
				  
				  $company_featured .=$wpjobus_company_services[$i][0].'</span>';
				  
				  
			  }
			  $company_featured .='</div></div>';
			  }
            $company_featured .='</div></div>';
			$t++;
			}
          ...
          $company_featured .='</div>';
			}
			
			$company_featured .='</div></div>
	</section>';
	
	$company_featured .='<script>';
	if($t==1)
	{
	$company_featured .='jQuery(document).ready(function() {
		 jQuery("#owl-car-1").addClass("owloneitem");
		 
		 
              jQuery("#owl-car-1").owlCarousel({
                loop: true,
                margin: 10,
                items: 2,
				responsive: {
 ...
 */
function wpdocs_registerprovider_settings_cb() {
    add_menu_page(
        __( 'Featured Providers Settings', 'textdomain' ),
        'Featured Providers Settings',
        'manage_options',
        'myplugin/myplugin-admin.php',
        'wpdocs_registerprovider_menu_settings_cb',
        '',
        6
    );
}
add_action( 'admin_menu', 'wpdocs_registerprovider_settings_cb' );
function wpdocs_registerprovider_menu_settings_cb()
{
	if(isset($_POST['featured_status_sub']))
	{
		update_option('featured_profile_status', $_POST['featured_profile_status']);
	}
	$featured_profile_status=get_option('featured_profile_status');
	$yesstatus='';
	$nostatus='';
	if($featured_profile_status=='yes')
	{
		$yesstatus='checked';
	}
	if($featured_profile_status=='no')
	{
		$nostatus='checked';
	}
	?>
	<h3>Home Page Featured Providers Settings</h3>
	<form method="post" action="" class="form_set_out">
	<h4><label>Do you want to show the Featured Providers Section on Home Page or Not?</label></h4>
	<label>Yes</label>
	<input type="radio" name="featured_profile_status" value="yes" <?php echo $yesstatus; ?> >
	
	<label>No</label>
	<input type="radio" name="featured_profile_status" value="no"  <?php echo $nostatus; ?>>
	<br>
	<br>
	<input type="submit" name="featured_status_sub">
	</form>
	<style>
	.form_set_out input[type="radio"] {
    margin: 0;
}
...
function isArrayEmpty($array) {
    foreach($array as $key => $val) {
        if (isArrayArrayEmpty($val))
            return true;
    }
    return false;
}
function isArrayArrayEmpty($array) {
    foreach($array as $key => $val) {
        if (!empty($val))
            return true;
    }
...
/*function check_current_role($rolename)
{
...
/*function wc_custom_remove_password_strength() {
 if ( wp_script_is( 'wc-password-strength-meter', 'enqueued' ) ) {
 wp_dequeue_script( 'wc-password-strength-meter' );
 }
}
add_action( 'wp_print_scripts', 'wc_custom_remove_password_strength', 100 );*/
